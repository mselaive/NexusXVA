...
import com.nexusxva.cva.application.CvaNettingSetCalculationCommand;
...
public record CvaNettingSetCalculationRequest(
        @NotNull UUID nettingSetId,
    ...
    CvaNettingSetCalculationCommand toCommand() {
    ...
    private List<CreditCurvePoint> toCreditCurve() {
        ...
        return creditCurve.stream().map(CreditCurvePointRequest::toDomain).toList();
    }
    ...
    public record DiscountCurvePointRequest(
            @NotNull LocalDate date,
            @NotNull @DecimalMin("0.0") @DecimalMax("1.0") Double discountFactor
    ) {
